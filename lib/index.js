...
function tryHighLightKotlin (wrapCode) {
  ...
  if (config.enabled) {
    ...
    const { prefix, hidden, suffix, ok, code: _code } = parseHide(code)
    if (ok) {
      return `<pre class="language-kotlin"><${tag} class="${tagClass}"${config.toString()}>${escapeHTML(prefix)}<textarea class="hidden-dependency">${escapeHTML(hidden)}</textarea>${escapeHTML(suffix)}</${tag}></pre>`
    } else {
      return `<pre class="language-kotlin"><${tag} class="${tagClass}"${config.toString()}>${escapeHTML(_code)}</${tag}></pre>`
    }
  } else {
